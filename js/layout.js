...
document.addEventListener('DOMContentLoaded', () => {
    ...
    style.textContent = `
        ...
        /* Sidebar styles - Using Darkest Premium Navy */
        #cx-sidebar {
            width: 260px;
            flex-shrink: 0;
            background: #020617;
    ...
    function renderLink(href, icon, label, viewOnly = false) {
        ...
        const activeClass = isActive
            ? 'bg-[#2DD4BF]/15 text-[#5EEAD4] border-[#2DD4BF]/50'
            : 'text-slate-300 hover:bg-[#1e293b] hover:text-white border-transparent';

        const activeIcon = isActive ? 'text-[#5EEAD4]' : 'text-slate-400 group-hover:text-white';
    ...
    const logoHTML = companyLogo
        ? `<img src="${companyLogo}" alt="${companyName}" class="size-8 rounded-lg object-contain bg-slate-100 p-0.5" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
           <div class="size-8 rounded-lg bg-[#2DD4BF] items-center justify-center text-white text-xs font-bold hidden">${companyInitials}</div>`
        : `<div class="size-8 rounded-lg bg-[#2DD4BF] flex items-center justify-center text-white text-xs font-bold">${companyInitials}</div>`;

    // 4. Create Sidebar Structure (only Contexia logo + text)
    const sidebar = document.createElement('aside');
    sidebar.id = 'cx-sidebar';
    sidebar.innerHTML = `
        <div class="h-16 shrink-0 flex items-center gap-3 px-6 border-b border-[#1e293b] bg-[#020617]">
        ...
        <div class="p-4 border-t border-[#1e293b] bg-[#020617]">
            <button onclick="window.auth.logout()" class="flex items-center gap-3 w-full px-3 py-2 text-slate-300 hover:text-white hover:bg-[#1e293b] rounded-lg transition-colors group">
                <span class="material-symbols-outlined group-hover:text-[#F97316] transition-colors">logout</span>
    ...
    topBar.innerHTML = `
        <div class="flex items-center gap-3 bg-slate-50 rounded-xl px-3 py-2 border border-slate-200">
            ...
            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#2DD4BF]/10 text-[#2DD4BF]">
                ${userRole}
            </span>
        </div>
    `;
