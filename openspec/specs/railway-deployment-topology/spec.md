# railway-deployment-topology Specification

## Purpose
Ensures Contexia's `antigravity-app` backend has exactly one documented canonical Railway deployment, that it runs with no empty required production secrets, and that any duplicate deployment is never decommissioned without prior verified end-to-end functionality on the canonical one. Established by `reconcile-railway-antigravity-projects` (2026-07-05) after discovering two Railway projects running the same service against the same database, and a related live production security gap (empty `JWT_SECRET` in production, caused by a validation function that was defined but never invoked).
