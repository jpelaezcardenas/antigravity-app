...
marketing hooks (default N=5, `{headline, body, cta, pain_tag}` each) via the LLM engine, and SHALL
fall back to a deterministic hook set if all LLM providers fail (never erroring the request).
