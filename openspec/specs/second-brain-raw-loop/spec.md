# second-brain-raw-loop Specification

## Purpose
Establishes the markdown capture/compilation foundation for Contexia's second brain: a separate `contexia-brain` repo (never inside `antigravity-app`, to avoid autonomous brain commits triggering production deploys), a `raw/` unstructured inbox, a Contexia-adapted MECE compiled-page structure, the interim manual librarian loop (superseded by GBrain's autonomous cycle once live — see `gbrain-adoption`), and a Stage-11-report harvest mechanism. Established by `adopt-gbrain-second-brain` (2026-07-05).
