# whatsapp-b2b-lead-signal Specification

## Purpose
Detects a B2B-shaped signal in inbound WhatsApp/Taty conversations and makes it visible to a
human operator (via `crm_leads.lead_type` and Chatwoot contact attributes), without provisioning
any tenant or altering the existing Renta Natural (B2C) funnel. Created by archiving change
`whatsapp-b2b-lead-bridge` (2026-09-09).
