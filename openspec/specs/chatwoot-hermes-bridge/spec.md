# chatwoot-hermes-bridge Specification

## Purpose

A thin, stateless FastAPI bridge (`apps/chatwoot-bridge/`) that connects a self-hosted Chatwoot
WhatsApp inbox to the local Hermes Gateway (Taty), running entirely on the founder's laptop for
data sovereignty. Filters/loop-prevents inbound events, respects a `bot_off` human-in-the-loop
pause, fetches conversation history, calls Hermes's OpenAI-compatible chat-completions API, and
dispatches the reply — degrading gracefully (never leaving the customer with no reply) when any
