...
The system SHALL run `gbrain sync --repo contexia-brain` so that brain markdown changes are re-indexed without manual re-import, via one of two mechanisms GBrain natively supports: `gbrain sync --watch [--interval N]` (a continuous, long-running watch process — verified present in `gbrain --help`, correcting an earlier draft of this spec that incorrectly assumed no watch mode existed) or `gbrain sync --install-cron` (a periodic daemon, e.g. every 10-15 minutes). Either satisfies this requirement.
