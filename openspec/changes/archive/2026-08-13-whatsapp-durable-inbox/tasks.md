...
- [x] 7.3 Confirmed 2026-08-13: `/inbox/health` returns 401 (not 404, so it's live and auth-gated).
      Sibling authenticated route `/inbox/pending` (same auth dependency, same service module) is
      continuously polled by the local `chatwoot-bridge` scheduled task and returns `200 OK` in
      production — see `reports/2026-08-13-deployment.md` for the full verification chain.
- [x] 7.4 Created: `openspec/changes/whatsapp-durable-inbox/reports/2026-08-13-deployment.md`

## 8. Archive

- [x] 8.1 Synced the capability into `openspec/specs/whatsapp-durable-inbox/spec.md` (new capability,
      no conflicts with existing specs) and archived to `openspec/changes/archive/2026-08-13-whatsapp-durable-inbox/`.
