...
- [x] 4.6 Mount the endpoint on a PWA-facing `pwa_router` at the `/radar` prefix, separate from the agent router at `/agents/radar-predictivo` — following the `centinela_endpoints` vs `centinela_agents_endpoints` split and `jarvis_endpoints`'s two-routers-per-module precedent. Added `TestRouteRegistration`, which asserts the real mounted path against `presentation.router.api_router` and reproduces the production 404 when the mount is wrong. See design.md Decision #8 — this was caught by the first production deploy, not by the tests, because every endpoint test called the handler function directly.
- [x] 4.5 Endpoint latency under 2s — **measured against the real production Supabase**, not mocks: Cliente Cero 1.222s, Lavadero de Carros 0.368s, CÓDIGO 520 0.202s. All under budget. **Caveat, stated honestly:** production Shadow GL currently holds only 30 journal entries / 60 lines across 14 tenants, so this measures the query round-trip, not behaviour at real data volume. Re-measure once ingestion populates real history.
...
- [x] 8.3 Verify against real Cliente Cero data — **done, executed `calculate_cash_projection_13w` against the real production Supabase** (tenant `e2d30d09-…df34`). Result: `estado: "sin_historico_suficiente"`, `metodologia: "solo_historico"`, `impuesto_futuro_estimado: null`, no `semanas`. That is the **correct** output, because Cliente Cero has **zero** rows in `erp_journal_entries`. Two tenants with the most data (Lavadero de Carros, CÓDIGO 520) return the same state — each has 3 entries across 2 distinct ISO weeks, below the 4-week floor. See the deployment report's "Shadow GL is effectively empty" finding.
...
- [x] 10.4 Production URL verified. The card is live at `contexia.online/app/radar` and renders. For an unauthenticated visitor it correctly degrades to its honest error state (endpoint returns 401, never a mock fallback). The "real chart with Cliente Cero figures" half was closed differently than planned — not through a logged-in browser session (this agent does not handle credentials) but by running the projection service directly against the real production Supabase (task 8.3). That produced the honest `sin_historico_suficiente` state, which is the correct rendering for that tenant today: **Cliente Cero has no Shadow GL history at all**. A logged-in session would show exactly the empty state, not a chart. Closed by founder decision.
...
- [x] 11.1 Confirm all tasks above are checked and Stage 11 report exists before archiving.
- [x] 11.2 Run the archive step once deployment is verified in production — never archive with Stage 11 incomplete.
