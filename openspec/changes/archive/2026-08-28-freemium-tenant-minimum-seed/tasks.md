...
- [x] 1.1 Write failing tests: seeds exactly one `-OPEN` entry + 2 lines (1110 debit / 3105
      credit) for the given tenant/nit/amount; idempotent on a second call (no duplicate); never
      touches another tenant's rows.
- [x] 1.2 Implement `seed_freemium_opening_balance(client, tenant_id, nit, name,
...
- [x] 2.1 Write failing tests: `plan_tier="freemium"` + `opening_balance_cents > 0` calls the seed
      ...
      succeeds.
- [x] 2.2 Add `opening_balance_cents: Optional[int] = None` parameter to `create_b2b_client`
...
- [x] 3.1 Add `opening_balance_cents: Optional[int] = None` to `CreateB2bClientRequest`
      (`apps/backend/presentation/crm_endpoints.py`), passed through to `create_b2b_client`.

## 4. Frontend

- [x] 4.1 Add optional "Saldo de apertura (COP)" numeric input to the alta form in
...
- [x] 5.1 Run full backend suite (`-k "crm or shadow_gl"` plus a broader sweep) — confirm no
      regressions.
- [x] 5.2 `tsc --noEmit` on `contexia-app/` — zero errors.

## 6. Reports

- [x] 6.1 Write `progress/impl_freemium-tenant-minimum-seed.md`.
...
- [x] 11.5 Create report: openspec/changes/freemium-tenant-minimum-seed/reports/YYYY-MM-DD-deployment.md
