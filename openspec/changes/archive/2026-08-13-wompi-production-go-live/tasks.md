...
- [ ] 6.1 **Still open — founder action required first.** Once Juan David reports the transaction
      (5.2), confirm via direct Supabase SQL that the corresponding `crm_wompi_transactions` row
      is `status="APPROVED"` with a genuine (non-`test_`-prefixed) `wompi_transaction_id`.
- [x] 6.2 Created deployment report at
      `openspec/changes/wompi-production-go-live/reports/2026-08-13-deployment.md`, documenting
      the config-flip completion, the `WOMPI_BASE_URL` dead-config observation, and the rollback
      plan. Explicitly does NOT claim the real-money verification happened.

## 7. Archive

- [x] 7.1 Synced `specs/wompi-production-go-live/spec.md` into `openspec/specs/` (new capability,
      no conflicts — note its third requirement describes the real-payment verification as an
      expected behavior, not a completion claim). **Archived with 5.1/5.2/6.1 deliberately left
      open** (2026-08-13 tech-debt triage, restoring the "one change at a time" invariant across
      11 accumulated changes). This does not fabricate or skip the real-payment gate — it remains
      tracked in `openspec/FOUNDER_ACTIONS_2026-08-13.md` as a founder follow-up.
