...
> **RECONCILIATION (2026-08-13, tech-debt triage):** every checkbox below was still `[ ]` despite
> 3 deployment reports (2026-06-25, 2026-06-25-final, 2026-06-26) claiming "100% complete, 42
> tests passing" — classic tracking drift, not a fabricated report. Verified today against the
> actual codebase, not just the reports' own claims:
>
> - **Stages 1-5 (schema, parser, persistence, endpoint, integration) — REAL, confirmed done.**
>   Migration `0019_shadow_gl_siigo_ingestion.sql` exists (tasks.md's own Stage 1 named a
>   different file, `001_shadow_gl_siigo_columns.sql`, that was never created — the actual
>   implementation used a different migration number, a real deviation from the plan, not a gap).
>   `parse_siigo_csv()` and `ingest_siigo_csv()` exist in `shadow_gl_service.py`. Both endpoints
>   exist: `POST /siigo-csv/ingest` (raw body, as originally planned) AND `POST
>   /siigo-csv/upload` (multipart, added later per the 2026-06-26 report's Stage 4). Ran the 4
>   test files the reports claim ("42 tests, 100% passing"): confirmed 42/42 green today
>   (2 additional failures in `test_shadow_gl_stage8_e2e.py` are a Windows cp1252-vs-UTF-8
>   file-read artifact in this shell, not a real regression — reproduced and diagnosed, not just
>   dismissed).
> - **Endpoint deployed to the CANONICAL 175a backend, not just dc78.** The 2026-06-26 report's
>   own "Production URL" only lists `antigravity-app-production-dc78.up.railway.app` — the
>   non-canonical, no-real-traffic backend per `ARCHITECTURE.md` Decision #9. Verified today:
>   `POST https://antigravity-app-production-175a.up.railway.app/api/v1/shadow-gl/siigo-csv/upload`
>   returns `422` (missing multipart body, not `404`) — the code is genuinely live on the
>   canonical backend that `contexia.online` actually routes to, the report's own URL just never
>   got updated to say so.
> - **Stage 6 (Documentation) — mostly done.** `docs/admin-runbook-shadow-gl.md` and
>   `docs/api-shadow-gl-ingestion.md` both exist. Task 6.3 (README.md section) was **not
>   done** — no "Shadow GL Data Ingestion" section exists in `README.md` as of this reconciliation.
> - **Stage 7 (Manual testing, real Cliente Cero data) — partially done.** A real Siigo export
>   fixture exists (`fixtures/contexia_siigo_journal_2026-06-18-to-2026-06-24.csv`). **No real XML
>   DIAN fixture exists** in this change's `fixtures/` directory — task 7.2 (XML upload with real
>   data) has no evidence it was ever executed against real Contexia data, only the CSV side was.
> - **Stage 8.1-8.2 (mypy --strict, ruff) — NOT run**, tooling unavailable in this environment
>   (`ruff` not installed; `mypy --strict` not attempted). Not claiming these passed when they
>   were never executed — genuinely unverified, left as `[ ]`.
> - **Stage 10 (deployment verification) — reconfirmed live today**: `GET /api/v1/health` on the
>   canonical 175a backend returns `{"status":"healthy",...}`.
>
> **Disposition:** archiving with Stages 1-5 + most of 6 + partial 7 + 9 + 10 marked done on
> verified evidence. Stage 6.3 (README section), the XML half of Stage 7, and Stage 8.1-8.2
> (type/lint checks) are genuinely incomplete — left unchecked below, not silently closed. None
> of these gaps affect production correctness (the code is live, tested, and serving the
> canonical backend); they are documentation/tooling debt, noted for whoever picks this back up.
