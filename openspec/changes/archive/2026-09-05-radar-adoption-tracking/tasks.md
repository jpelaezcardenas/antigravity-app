...
Tasks:
- [x] 6.1 git commit + push to main — `353dc7e`
- [x] 6.2 Railway deploy active — deployment `0d70e48c` SUCCESS; `/api/v1/health` 200 and `/api/v1/radar/proyeccion-caja` still 401 without a token, so the telemetry write did not alter the endpoint contract
- [x] 6.3 Migration `0047` applied to production Supabase. Additive only (`CREATE TABLE IF NOT EXISTS` + indexes + policies); no existing table or row touched; rollback is `DROP TABLE public.radar_module_opens`. Applied under the founder's standing instruction to finish the outstanding work; flagged here so it is visible rather than silent.
- [x] 6.4 Confirmed against the real database: 1 call = 1 row, 3 calls = still 1 row, cleanup 0. This is what surfaced the two silent-no-op bugs (anon client vs `user_roles` policy recursion; `ON CONFLICT` cannot infer a partial index).
- [x] 6.5 Report: `openspec/changes/radar-adoption-tracking/reports/2026-09-05-deployment.md`

## 7. Archive

- [x] 7.1 Archive once Stage 11 is verified.
