...
- [ ] 8.1 git commit + push to `main` in `contexia-wizard`
- [ ] 8.2 Verify Vercel build for `contexia-wizard.vercel.app` is green
- [ ] 8.3 Production check: visit `contexia.online/wizard/iva-ecom` (through the `antigravity-app` rewrite, hard refresh) and confirm the full 3-step flow works in production
- [ ] 8.4 Confirm `contexia.online/wizard` (existing 8-step flow) still works unchanged in production
- [ ] 8.5 Create deployment report: `openspec/changes/wizard-iva-ecom-express-diagnostic/reports/YYYY-MM-DD-deployment.md`
