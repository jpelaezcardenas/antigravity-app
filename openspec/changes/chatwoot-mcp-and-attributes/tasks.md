...
- [ ] 4.3 **FOUNDER ACTION, not completed tonight**: supply a `CHATWOOT_ADMIN_TOKEN` (Administrator-role Chatwoot API token, or create the attributes once via Settings → Custom Attributes in the Chatwoot UI) so `chatwoot_create_custom_attribute_definition` can actually run — see design.md Decision #2. Taty Bot's existing agent-role token returns 403 for this one operation by design; not a bug.
- [ ] 4.4 **FOUNDER ACTION**: once 4.3 unblocks, provision the full attribute schema from design.md Decision #5 (9 contact-level + 7 conversation-level attributes)
