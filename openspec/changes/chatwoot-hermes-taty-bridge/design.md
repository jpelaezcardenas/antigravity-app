...
5. **New-lead intake reuses, not duplicates, CRM state.** `POST /api/v1/crm/leads/whatsapp-intake` does a tenant-scoped find-or-create against the same `leads` table `b2c_pipeline`/`advance_lead` already operate on (`crm_service.py`), keyed on normalized `whatsapp_phone`. On creation, it triggers the existing `POST /api/v1/social-ops/onboarding/start` rather than the bridge calling onboarding directly — the backend is the single owner of lead lifecycle; the bridge only observes and reports the WhatsApp contact event.
