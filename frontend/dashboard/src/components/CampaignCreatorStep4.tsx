...
export const CampaignCreatorStep4: React.FC = () => {
  ...
  const generateContent = async () => {
    ...
    try {
      const response = await fetch('/api/v1/agents/generator/create-content', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          campaign: selectedOption,
          tax_dna: taxDna,
          channel: selectedOption?.channels?.[0] || 'instagram',
          variations_count: 3,
        }),
      });
      ...
      await reviewContent(data.variations);
    } catch (err) {
      setPublishError(err instanceof Error ? err.message : 'Failed to generate content');
    }
  };
  ...
  const reviewContent = async (content?: any[]) => {
    ...
    try {
      const contentToReview = content || generatedContent;
      const response = await fetch('/api/v1/agents/orchestrator/review-content', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          content: { variations: contentToReview },
          tax_dna: taxDna,
          language: 'es',
          strict_mode: false,
        }),
      ...
      setReviewResult({
        ...
        approved_content: data.approved_content,
        disclaimers_added: data.disclaimers_added || [],
      ...
      setPublishError(err instanceof Error ? err.message : 'Failed to review content');
      setStep('preview');
    }
  };
      ...
      {step !== 'preview' && (
        <div className="flex items-center justify-center py-12">
          <div className="text-center">
            ...
            <p className="text-gray-600">
              {step === 'generating' ? 'Generating content variations...' : 'Reviewing content for compliance...'}
            </p>
          </div>
        </div>
      )}
