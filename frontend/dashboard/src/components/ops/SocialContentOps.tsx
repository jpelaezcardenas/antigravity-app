...
const urgencyStyles: Record<string, string> = {
  low: 'border-slate-600/60 bg-slate-800/50 text-slate-200',
...
const stageAccent: Record<string, string> = {
  ...
  diagnosticado: 'border-cyan-500/40',
...
const SocialContentOps: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="grid min-h-[360px] place-items-center rounded-lg border border-slate-700 bg-slate-950/60">
        <p className="text-sm uppercase tracking-[0.2em] text-cyan-300">Cargando Social Content Ops</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-white/10 bg-gradient-to-r from-slate-950 via-slate-950 to-cyan-950/30 p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-[11px] uppercase tracking-[0.24em] text-cyan-300">Bunker operativo</p>
            <h2 className="mt-1 text-2xl font-bold text-white">Social Media OPs Systems</h2>
            <p className="mt-1 max-w-3xl text-sm text-cyan-200/70">
              Motor de contenido orgánico para Facebook, Instagram, TikTok, LinkedIn y Telegram — Contexia.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            {CHANNELS.map((channel) => (
              <Badge key={channel.value} className="border-cyan-500/30 bg-cyan-500/10 text-cyan-200">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-5">
          <Stat label="Leads activos" value={activeLeads.length} tone="border-cyan-500/30 bg-cyan-500/10" />
          ...
          {LEGACY_TABS.map((tab) => (
            <button
              ...
              className={`rounded-xl px-4 py-3 text-sm font-semibold transition ${
                activeLegacy === tab.id ? 'bg-cyan-500/15 text-cyan-200 border border-cyan-400/20' : 'text-slate-300 hover:bg-white/5 border border-transparent'
              }`}
            >
              {tab.label}
            </button>
        ...
        <div className="grid gap-5 xl:grid-cols-[380px_minmax(0,1fr)]">
          <form onSubmit={handleSimulate} className="space-y-4 rounded-lg border border-slate-700 bg-slate-950/60 p-4">
            <div>
              ...
              <p className="text-xs text-slate-500">Smoke test local de eventos normalizados.</p>
            </div>
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Canal</span>
              <select
            ...
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Handle</span>
              <input
            ...
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Mensaje</span>
              <textarea
                value={simulateForm.text}
                onChange={(event) => setSimulateForm((prev) => ({ ...prev, text: event.target.value }))}
                rows={5}
                className="mt-1 w-full resize-none rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-white outline-none focus:border-cyan-400"
              />
            </label>
            <button className="w-full rounded-md bg-cyan-500 px-4 py-2 text-sm font-bold text-slate-950 hover:bg-cyan-300">
              Crear evento
            </button>
            {lastDiagnosis && (
              <div className="rounded-md border border-cyan-500/30 bg-cyan-500/10 p-3 text-sm text-cyan-100">
                ...
                <p className="mt-1 text-xs text-cyan-200/80">{lastDiagnosis.safe_reply}</p>
              </div>
            )}
          </form>
          ...
          <div className="space-y-3">
            {inbox.length === 0 ? (
              <div className="rounded-lg border border-slate-700 bg-slate-950/60 p-8 text-center text-slate-400">
                Inbox sin eventos.
              </div>
            ) : (
              inbox.map((event) => (
                <article key={event.id} className="rounded-lg border border-slate-700 bg-slate-950/60 p-4">
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm leading-6 text-slate-300">{event.text}</p>
                    </div>
                    <div className="flex flex-col gap-2">
                      <button
                        onClick={() => handleDiagnose(event.id)}
                        className="rounded-md border border-cyan-500/40 px-3 py-2 text-xs font-semibold text-cyan-200 hover:bg-cyan-500/10"
                      >
                        Diagnosticar
                      </button>
                      <button
                  ...
                  {event.pain_tags.length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-2">
                      {event.pain_tags.map((tag) => (
                        <Badge key={tag} className="border-slate-600 bg-slate-800 text-slate-300">
                          {painLabel(tag)}
                        </Badge>
                      ))}
                    </div>
          ...
          {pipeline?.columns.map((column) => (
            <section key={column.id} className={`rounded-lg border ${stageAccent[column.id] || 'border-slate-700'} bg-slate-950/60 p-3`}>
              <div className="flex items-center justify-between">
                ...
                <span className="rounded-full bg-slate-800 px-2 py-0.5 text-xs text-slate-300">{column.leads.length}</span>
              </div>
              <div className="mt-3 space-y-3">
                {column.leads.length === 0 ? (
                  <p className="rounded-md border border-dashed border-slate-700 p-3 text-xs text-slate-500">Sin leads.</p>
                ) : (
                    ...
                    <article key={lead.id} className="rounded-md border border-slate-700 bg-slate-900/70 p-3">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{lead.actor_handle}</p>
                        </div>
                        <Badge className={urgencyStyles[lead.urgency] || urgencyStyles.low}>{lead.urgency}</Badge>
                      </div>
                      <p className="mt-2 text-xs text-cyan-200">{lead.maturity_stage}</p>
                      <p className="mt-2 text-xs leading-5 text-slate-400">{lead.last_message}</p>
                      <div className="mt-3 grid grid-cols-2 gap-2">
                        <button
                          onClick={() => handleDraftReply(lead.id, lead.primary_channel)}
                          className="rounded-md border border-cyan-500/40 px-2 py-1.5 text-xs font-semibold text-cyan-200 hover:bg-cyan-500/10"
                        >
                          Draft reply
                        </button>
                        <button
      ...
      {lastOutboundDraft && (
        <section className="rounded-lg border border-slate-700 bg-slate-950/60 p-4">
          ...
          <p className="mt-3 rounded-md border border-slate-700 bg-slate-900 p-3 text-sm leading-6 text-slate-200">
            {lastOutboundDraft.message_text}
          </p>
          <p className="mt-2 text-xs text-slate-500">{lastOutboundDraft.approval_reason}</p>
        </section>
      )}
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,460px)_minmax(0,1fr)]">
          <form onSubmit={handleCommand} className="space-y-4 rounded-lg border border-slate-700 bg-slate-950/60 p-4">
            <div>
              ...
              <p className="text-xs text-slate-500">Los comandos quedan como borradores aprobables.</p>
            </div>
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Canal</span>
              <select
            ...
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Operador</span>
              <input
            ...
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Comando</span>
              <textarea
            ...
            <button className="w-full rounded-md bg-amber-400 px-4 py-2 text-sm font-bold text-slate-950 hover:bg-amber-300">
              Crear borrador
            </button>
          </form>
            ...
            {lastDraft ? (
              <div className="mt-4 space-y-3">
                <div className="flex flex-wrap gap-2">
                  ...
                  <Badge className="border-cyan-500/30 bg-cyan-500/10 text-cyan-200">{lastDraft.action}</Badge>
                  <Badge className="border-slate-600 bg-slate-800 text-slate-300">{lastDraft.risk_level}</Badge>
                </div>
                <p className="rounded-md border border-slate-700 bg-slate-900 p-3 text-sm leading-6 text-slate-300">
                  {lastDraft.command_text}
                </p>
                <p className="text-sm text-slate-400">{lastDraft.approval_reason}</p>
              </div>
            ) : (
              <p className="mt-4 text-sm text-slate-500">Sin borrador seleccionado.</p>
            )}
          </section>
        </div>
      )}
      ...
      {activeTab === 'approvals' && (
        <section className="rounded-lg border border-slate-700 bg-slate-950/60 p-4">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Aprobador</span>
              <input
                value={approvalActor}
            ...
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Motivo rechazo</span>
              <input
              ...
              approvals.map((item) => (
                <div key={`${item.type}:${item.id}`} className="rounded-md border border-slate-700 bg-slate-900/60 p-3">
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <Badge className="border-cyan-500/30 bg-cyan-500/10 text-cyan-200">{item.type}</Badge>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="rounded-md bg-emerald-400 px-3 py-2 text-xs font-bold text-slate-950 hover:bg-emerald-300"
                      >
                        Aprobar
                      </button>
                      <button
            ...
            <article key={account.id} className="rounded-lg border border-slate-700 bg-slate-950/60 p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-2 text-sm text-slate-400">{account.notes}</p>
                </div>
                <Badge className="border-slate-600 bg-slate-800 text-slate-300">{account.auth_status}</Badge>
              </div>
