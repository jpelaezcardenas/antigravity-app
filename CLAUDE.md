...
### EXCEPTION — live Caja Real wiring is hand-edited in `app/` (2026-06-30/07-01)

There is ONE deliberate, documented exception to "never hand-edit `app/`":
the live Caja Real data wiring lives as a hand-injected `<script>` at the end of
`app/overview.html` (commit `f91d9da`), plus the corrected logout label
"Cerrar Sesión" (commit `8559ca1`) and the SW `CACHE_VERSION` bump. **This is in
production and must not be reverted.**

**Why the exception exists:** the full end-user PWA (Pulso header nav, Auditoría
Sombra CTA, Taty card, all screen bodies) currently exists ONLY as this
pre-built static export in `app/`. The `contexia-app/` source renders
placeholders for 4 of 5 screens and **cannot reproduce this UI** — so a
`npm run build` + sync of `contexia-app/out/` → `app/` would silently overwrite
the real client UI AND wipe the live Caja Real, rolling production back to the
baked-in mock `$42.850.000`.

**Hard rule for any session:** do NOT regenerate `app/` from `contexia-app/`, and
do NOT revert the injected `<script>` in `app/overview.html`, until the durable
follow-up is done: reconcile the `contexia-app/` source so a rebuild reproduces
this exact UI *with* the live-data fetch baked in. Until then, `app/overview.html`
is edited by hand on purpose. (Full context: user memory
`pwa-correct-version-restored-2026-06-30`.)
