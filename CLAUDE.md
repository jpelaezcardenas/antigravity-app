...
### Caja Real wiring — RECONCILED (2026-07-01)

**Status:** ✅ **EXCEPTION RETIRED** — live Caja Real source is now fully reconciled.

The live Caja Real data fetch is **integrated into the React component hierarchy**
via `CashTodayCard` (`components/pulso/CashTodayCard.tsx`):
- `useEffect` + `fetchFinancials()` on mount
- Explicit loading/ready/empty/error states  
- Graceful fallback (never shows error banner)
- Backend minor units (cents) → whole COP conversion

The **hand-edited `<script>` in `app/overview.html` has been retired.** It is no longer needed.

The **full end-user PWA** (Pulso header nav + Auditoría Sombra CTA + Taty card + all screen
bodies + live Caja Real data) is now **fully reproducible from `contexia-app/` source**
via `npm run build` + sync. `app/` is once again a clean build artifact.

**Hard rule for any session:** `npm run build` from `contexia-app/` now produces the exact
live UI. Never hand-edit `app/` — it is a build artifact. (Reconciliation: OpenSpec change
`reconcile-contexia-app-source-live-pwa`, commits d95ce65+.)
